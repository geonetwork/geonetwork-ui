...
describe('DownloadsListComponent', () => {
  ...
  describe('with an empty list of downloads', () => {
    ...
    beforeEach(() => {
      component.listItems = []
      fixture.detectChanges()
      item = de.query(By.directive(ListComponent))
    })
  ...
  describe('when link format is unknown', () => {
    ...
    beforeEach(() => {
      component.listItems = [LINK_FIXTURES.unknownFormat]
      fixture.detectChanges()
      items = de.queryAll(By.directive(MockDownloadItemComponent))
    })
    it('contains one link in "others" section', () => {
      expect(items.length).toBe(1)
      expect(component.isLinkOfFormat(component.listItems[0], 'others')).toBe(
        true
      )
    })
  })
  describe('when link mime type is unknown', () => {
    ...
    it('contains one link and mime type is ignored', () => {
      expect(items.length).toBe(1)
      expect(component.isLinkOfFormat(component.listItems[0], 'json')).toBe(
        true
      )
    })
  })
  describe('derives color and format from link', () => {
    ...
    beforeEach(() => {
      component.listItems = [LINK_FIXTURES.geodataShpWithMimeType]
      fixture.detectChanges()
      items = de.queryAll(By.directive(MockDownloadItemComponent))
    })
  ...
  describe('filtering links', () => {
    ...
    describe('no filter', () => {
      ...
      it('shows all links', () => {
        expect(component.filteredListItems).toEqual([
          LINK_FIXTURES.dataCsv,
          LINK_FIXTURES.geodataJsonWithMimeType,
        ])
      })
    ...
    describe('filter on csv', () => {
      ...
      it('shows only one link', () => {
        expect(component.filteredListItems).toEqual([LINK_FIXTURES.dataCsv])
      })
    })
  ...
  describe('filter buttons visibility', () => {
    ...
    describe('pdf', () => {
      beforeEach(() => {
        component.listItems = [LINK_FIXTURES.dataPdf]
        fixture.detectChanges()
        items = de.queryAll(By.css('.format-filter'))
      })
