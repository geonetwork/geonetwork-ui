...
import { Engine } from './lib/engine/duckdb'
...
jest.mock('./lib/engine/duckdb', () => {
  ...
  return {
    ...originalModule,
    async getEngine() {
      const engine = new ReplacedEngine()
      allEngines.push(engine as Engine)
      await engine.init()
      return engine
    },
  }
...
// cleanup all started engines
afterAll(() => {
  allEngines.forEach((engine) => engine.close())
})
