...
- `[[custom_filter]]` (multiple, optional)
  ...
  - `base_filter` (mandatory): the search field the filter is based on; supported values are `'format'`, `'representationType'`, `'publicationYear'`, `'topic'`, `'inspireKeyword'`, `'keyword'`, `'documentStandard'`, `'license'`, `'producerOrg'`, `'publisherOrg'` and `'user'`. The other search fields cannot be used as a base filter, either because they are not built on a single Elasticsearch field (`'organization'`, `'q'`), because they write to other filters (`'recordKind'`, `'availableServices'`), because only one of them can be applied to a search (`'spatialExtent'`), because they offer no list of values to restrict (`'owner'`, the date ranges `'changeDate'` and `'resourceCreationRevisionDate'`, and `'isSpatial'` which is a single choice between two mutually exclusive values), or because they are deprecated (`'resourceType'`)
