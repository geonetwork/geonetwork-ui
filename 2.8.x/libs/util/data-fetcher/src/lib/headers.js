...
function parseHeaders(httpHeaders) {
    ...
    if (httpHeaders.has('Content-Type')) {
        ...
        const supported = model_1.SupportedTypes.filter((type) => type !== 'wfs') // Ignore wfs type as it is not a file type
            .filter((type) => model_1.AllMimeTypes[type].indexOf(result.mimeType) > -1)[0] || null;
        if (supported !== null)
