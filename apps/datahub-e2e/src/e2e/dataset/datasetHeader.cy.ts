describe('dataset: header', () => {
  ...
  it('should display the navigation bar on mobile', () => {
    ...
    // FIXME: could not get this to work in cypress since cdk overlay does not become visible
    // it should expand mobile menu when clicking on the toggle
    // cy.get('datahub-navigation-bar')
    //   .find('[data-cy="navbar-mobile-toggle"]')
    //   .click()
    // cy.get('datahub-navigation-bar')
    //   .find('[data-cy="resources-mobile-menu"]')
    //   .should('be.visible')
