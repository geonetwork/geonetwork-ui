describe('filters and sorts', () => {
  ...
  describe('Filter on type', () => {
    describe('When there is no filter on type', () => {
    ...
    describe('When filtering on types', () => {
      it('should select the value, update the url, and update the search results', () => {
        ...
        cy.get('@inlineFilter-service').should('not.be.checked')
        cy.get('@inlineFilter-reuse').should('not.be.checked')
        cy.url().should('contain', 'type=dataset')
        ...
        cy.get('@inlineFilter-reuse').should('not.be.checked')
        cy.url().should('contain', 'type=dataset,service')
        cy.get('[data-cy="resultsHitsFound"]').should('contain.text', '25 ')
    ...
    describe('When toggling on types', () => {
