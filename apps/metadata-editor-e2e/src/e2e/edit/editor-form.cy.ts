...
describe('editor form', () => {
  ...
  it('form display', () => {
    // opens the first page by default
  ...
  describe('form fields', () => {
    ...
    it('licenses & constraints', () => {
      ...
      // should select a new license and show it on reload
      ...
      // should add a few constraints and show it on reload
      cy.editor_wrapPreviousDraft(recordUuid)
      ...
      // should enable "no applicable constraints" and stay enabled
      cy.editor_wrapPreviousDraft(recordUuid)
    ...
    it('contacts for metadata & for resource', () => {
      cy.get('@accessContactPageBtn').click()

      // show the contacts for resource of the dataset
      cy.get('[data-test=displayedRoles]').children().should('have.length', 3)

      // delete a contact for resource
      ...
      // show the roles available to add
      cy.get('[data-test=rolesToPick]').children().should('have.length', 18)

      // click on a role adds it to the list of displayed role
      cy.get('[data-test="rolesToPick"]').children().eq(2).click()
      cy.get('[data-test=rolesToPick]').children().should('have.length', 17)
      cy.get('[data-test=displayedRoles]').children().should('have.length', 2)

      // add a contact for resource
      cy.get('[data-test=displayedRoles]')
        .children()
        .find('gn-ui-contact-card')
        .should('have.length', 1)
